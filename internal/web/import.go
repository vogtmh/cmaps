...
func (app *Server) ImportFromMySQL(c MySQLConfig, report func(stage string, res ImportResult)) (*ImportResult, error) {
...
func (app *Server) ImportAuditOnly(c MySQLConfig) (int, error) {
	dsn := fmt.Sprintf("%s:%s@tcp(%s:%s)/%s?charset=utf8mb4&parseTime=false",
		c.User, c.Password, c.Host, c.Port, c.Database)
	sqldb, err := sql.Open("mysql", dsn)
	if err != nil {
